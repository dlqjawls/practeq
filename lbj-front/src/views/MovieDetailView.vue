...
const loadComments = async () => {
  try {
    const response = await axios.get(`http://3.36.40.72:8000/movies/${props.movieId}/comment/`);
...
const submitComment = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      `http://3.36.40.72:8000/movies/${props.movieId}/comment/`,
...
const deleteComment = async (commentId) => {
  try {
    const token = JSON.parse(localStorage.getItem('counter')).token;
    await axios.delete(`http://3.36.40.72:8000/movies/${props.movieId}/comment/${commentId}/`, {
      headers: { Authorization: `Token ${token}` },
    });
...
const editComment = async (commentId, newContent) => {
  try {
    ...
    const response = await axios.put(
      `http://3.36.40.72:8000/movies/${props.movieId}/comment/${commentId}/`,
      { content: newContent },
      { headers: { Authorization: `Token ${token}` } }
    );
